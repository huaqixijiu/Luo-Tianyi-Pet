...
public partial class MainWindow
{
    ...
    private async Task RunPlannerQaAsync()
    {
        ...
        try
        {
            ...
            await service.ChangeAsync(b => {
                b.Items.Clear(); b.WeekView=false;
                for(int i=0;i<3;i++) b.Items.Add(new ReminderItem { Calendar=true, Title=new[]{"项目讨论","家庭聚餐","整理资料"}[i], Content=new[]{"项目讨论\n准备演示资料，确认下一阶段安排。","家庭聚餐","整理资料"}[i], Repeat=ReminderRepeat.Dates, Start=DateTime.Today.AddDays(i+1).AddHours(14), Dates=[DateTime.Today.AddDays(i+1),DateTime.Today.AddDays(i+8),DateTime.Today.AddDays(i+15)] });
                b.Items.Add(new ReminderItem { Title="起床", Start=DateTime.Today.AddDays(1).AddHours(8.5), Repeat=ReminderRepeat.Weekly, Weekdays=[DayOfWeek.Monday,DayOfWeek.Tuesday,DayOfWeek.Wednesday,DayOfWeek.Thursday,DayOfWeek.Friday] });
                b.Items.Add(new ReminderItem { Title="休息一下", Start=DateTime.Now.AddMinutes(25), Relative=true, DurationSeconds=1500 });
            });
            typeof(PlannerWindow).GetField("_date",BindingFlags.NonPublic|BindingFlags.Instance)!.SetValue(window,DateTime.Today.AddDays(1));
            typeof(PlannerWindow).GetField("_batch",BindingFlags.NonPublic|BindingFlags.Instance)!.SetValue(window,false);
            window.Navigate(false); Capture("redesign-month");
            await service.ChangeAsync(b=>b.WeekView=true); Capture("redesign-week");
            window.Navigate(true); Capture("redesign-alarms"); window.Navigate(false);
            var group=service.Book.Items.First();
            typeof(PlannerWindow).GetField("_occurrenceDate",BindingFlags.NonPublic|BindingFlags.Instance)!.SetValue(window,group.Start.Date);
            typeof(PlannerWindow).GetMethod("Edit",BindingFlags.NonPublic|BindingFlags.Instance)!.Invoke(window,new object?[]{group,true}); Capture("redesign-editor");
            ((System.Windows.Controls.Button)Named(window,"DeleteOccurrence")).RaiseEvent(new RoutedEventArgs(System.Windows.Controls.Button.ClickEvent)); window.UpdateLayout(); Capture("delete-one-confirm");
            ((System.Windows.Controls.Button)Named(window,"ConfirmGroupDelete")).RaiseEvent(new RoutedEventArgs(System.Windows.Controls.Button.ClickEvent)); await Task.Delay(250);
            if(service.Book.Items.First().Dates.Count!=2 || service.Book.Items.Count!=5) throw new InvalidOperationException("Single-date delete removed wrong scope");
            group=service.Book.Items.First();
            typeof(PlannerWindow).GetMethod("Edit",BindingFlags.NonPublic|BindingFlags.Instance)!.Invoke(window,new object?[]{group,true}); window.UpdateLayout();
            ((System.Windows.Controls.Button)Named(window,"DeleteGroup")).RaiseEvent(new RoutedEventArgs(System.Windows.Controls.Button.ClickEvent)); window.UpdateLayout(); Capture("delete-group-confirm");
            ((System.Windows.Controls.Button)Named(window,"ConfirmGroupDelete")).RaiseEvent(new RoutedEventArgs(System.Windows.Controls.Button.ClickEvent)); await Task.Delay(250);
            if(service.Book.Items.Any(i=>i.Id==group.Id) || service.Book.Items.Count!=4) throw new InvalidOperationException("Whole-group deletion failed");
            typeof(PlannerWindow).GetField("_manage",BindingFlags.NonPublic|BindingFlags.Instance)!.SetValue(window,true);
            var chosen=(HashSet<Guid>)typeof(PlannerWindow).GetField("_selectedGroups",BindingFlags.NonPublic|BindingFlags.Instance)!.GetValue(window)!;
            foreach(var item in service.Book.Items.Where(i=>i.Calendar)) chosen.Add(item.Id);
            typeof(PlannerWindow).GetMethod("Render",BindingFlags.NonPublic|BindingFlags.Instance,null,Type.EmptyTypes,null)!.Invoke(window,null); Capture("group-manager");
            ((System.Windows.Controls.Button)Named(window,"DeleteSelectedGroups")).RaiseEvent(new RoutedEventArgs(System.Windows.Controls.Button.ClickEvent)); window.UpdateLayout();
            ((System.Windows.Controls.Button)Named(window,"ConfirmGroupDelete")).RaiseEvent(new RoutedEventArgs(System.Windows.Controls.Button.ClickEvent)); await Task.Delay(250);
            var persisted=await new ReminderStore(new LocalAppPaths(Path.Combine(path,"UserData"))).LoadAsync();
            if(persisted.Items.Count!=2 || persisted.Items.Any(i=>i.Calendar)) throw new InvalidOperationException("Bulk group deletion did not preserve unrelated alarms");
            checks.Add("PASS actual single-date, whole-group and bulk-group deletion persist correct scope");
